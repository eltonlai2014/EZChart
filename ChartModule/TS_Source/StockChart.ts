...
import { BaseChart, TextObj, MyMouseEvent, HashMap } from "./Helper/index";
    ...
    public drawFrontContext = (mouseEvent?: MyMouseEvent): void => {
        ...
        if (xIndex >= 0 && xIndex < this.mChartData.length) {
            ...
            if (this.mShowQuoteLine) {
                ...
                let xAlpha = 0.7;
                let xRadius = 5;
                let aInfo = this.mChartData[xIndex];
                let aColor = this.hexToRgb(this.mRealColor);
                let yPos = this.getYPosByType(aInfo, ChartType.REAL_CHART);
                this.drawCircle(this.mContext, LineX, yPos, xRadius, 'rgba(' + aColor.r + ',' + aColor.g + ',' + aColor.b + ',' + xAlpha + ')');
                aColor = this.hexToRgb(this.mInvColor);
                yPos = this.getYPosByType(aInfo, ChartType.INV_CHART);
                this.drawCircle(this.mContext, LineX, yPos, xRadius, 'rgba(' + aColor.r + ',' + aColor.g + ',' + aColor.b + ',' + xAlpha + ')');
